...
{
    matrix World : packoffset(c0);//vs
  
};



Texture2D				texHeightMap	: register( t0 );//DS
Texture2D				texLODLookup	: register( t1 );//HS
SamplerState			smpHeightMap	: register( s0 );

cbuffer patch : register(b1) //hs
{
    float4 cameraPosition : packoffset(c0); //  w unused 
    float4 minMaxDistance : packoffset(c1); // zw unused
    float4 minMaxLODHS : packoffset(c2); // zw unused
    float4 heightMapDimensionsHS : packoffset(c3);
}

cbuffer sampleparams : register(b2)//ds
{
    matrix ViewProj : packoffset(c0);
    matrix InvTposeWorld : packoffset(c4);
    float4 heightMapDimensionsDS : packoffset(c8);
    float4 minMaxLODDS : packoffset(c9);
}
...
{	
	...
	return lerp( minMaxLODHS.x, minMaxLODHS.y, 1.0f - d );
}
...
{
	// Useful aliases
	float2 pxSz = float2( 1.0f / heightMapDimensionsDS.x, 1.0f / heightMapDimensionsDS.y );
...
uint2 ComputeLookupIndex(uint patch, int xOffset, int zOffset)
{
	...
	p.x = patch % (uint)heightMapDimensionsHS.z;
	p.y = patch / (uint)heightMapDimensionsHS.w;
	
	// With the XY coordinate for the patch being rendered we
	// then need to offset it according to the parameters
	p.x = clamp(p.x + xOffset, 0, ((uint)heightMapDimensionsHS.z) - 1);
	p.y = clamp(p.y + zOffset, 0, ((uint)heightMapDimensionsHS.w) - 1);
...
float ComputePatchLODUsingLookup(float3 midPoint, float4 lookup)
{
	...
	return lerp( minMaxLODHS.x, minMaxLODHS.y, clamp(lod, 0.0f, 1.0f) );
}
...
VS_OUTPUT vsMain( in VS_INPUT v )
{
	...
	o.position = mul( float4( v.position, 1.0f ), World ).xyz;
...
[domain("quad")]
DS_OUTPUT dsMain( HS_PER_PATCH_OUTPUT ip, 
                        ...
                        const OutputPatch<HS_OUTPUT, 4> patch )
{
    ...
    */
    ...
    o.position = mul( float4( finalVertexCoord, 1.0f ), ViewProj );
    
  //  #if defined( SHADING_SOLID )
    
		//o.colour = float3( 1.0f, 0.0f, 0.0f );
    
  //  #elif defined( SHADING_SIMPLE )
    
		// Perform a sobel filter on the heightmap to determine an appropriate normal  vector
   
		//float3 normal = Sobel(texcoord);
  //      normal = normalize(mul(float4(normal, 1.0f), InvTposeWorld).xyz);
  //      o.colour = min(0.75f, max(0.0f, dot(normal, float3(0.0f, 1.0f, 0.0f))));
    
  //  #elif defined( SHADING_DEBUG_LOD )
    
    float lod = (ip.insideTesselation[0] - minMaxLODDS.x) / (minMaxLODDS.y - minMaxLODDS.x);
...
[maxvertexcount(3)]
void gsMain(triangle DS_OUTPUT ip[3], inout TriangleStream<DS_OUTPUT> TriangleOutputStream)
{
    TriangleOutputStream.Append(ip[0]);
    TriangleOutputStream.Append(ip[1]);
    TriangleOutputStream.Append(ip[2]);
...
PS_GBUFFER_OUT psMain(in DS_OUTPUT f)
{
   
    PS_GBUFFER_OUT Out;
    Out.ColorSpecInt = float4(f.colour, 0);
    Out.Specular = float4(0, 0, 0.0, 0.0);
    Out.Normal = float4(0,0,0, 1.0);
    return Out;
}