...
float4 FinalPassPS(VS_OUTPUT In) : SV_TARGET
{
    ...
    //float R = HDRTex.GatherRed(PointSampler, In.UV).r;
    //float G = HDRTex.GatherGreen(PointSampler, In.UV).r;
    //float B = HDRTex.GatherBlue(PointSampler, In.UV).r;
    //float3 color = float3(R, G, B);
   
    //R = BloomTex.GatherRed(LinearSampler, In.UV).r;
    //G = BloomTex.GatherGreen(LinearSampler, In.UV).r;
    //B = BloomTex.GatherBlue(LinearSampler, In.UV).r;
    //float3 bloom = float3(R, G, B);
